        ...
        for ch in channels:
            ...
            while time.monotonic() < end:
                ...
                for desc, payload in self.iter_rx_payloads(urb):
                    ...
                    ssid, ch_ie = _extract_mgmt_ssid_and_channel(payload)
                    if ssid is None:
                        continue
                    ...
                    row = results.get(key)
                    if row is None:
                        results[key] = {"bssid": bssid, "ssid": ssid, "channel": ch_eff, "seen": 1, "tsfl": desc.tsfl}
                    else:
        ...
        if args.scan:
            ...
            for r in rows:
                ...
                if ssid == "":
                    ssid = "<hidden>"
                sys.stdout.write(
                    f"ch={int(r['channel']):02d} bssid={r['bssid']} seen={int(r['seen'])} ssid={ssid}\n"
                )
            sys.stdout.flush()
